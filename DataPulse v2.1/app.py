...
class ModbusApp:
    def __init__(self, root):
        ...
        self.colors = {'light': {'success': 'green', 'error': 'red', 'info': 'blue', 'normal': 'black'}, 'dark':  {'success': '#4CAF50', 'error': '#F44336', 'info': '#2196F3', 'normal': 'white'}}
        ...
        # --- YENİ ALARM DEĞİŞKENLERİ ---
        self.is_scanning_active = False; self.scanning_thread = None
        self.is_alarm_engine_active = False
        self.alarm_engine_thread = None
        self.active_alarms = {}  # Aktif alarmları ID'leriyle saklamak için
    ...
    def _create_tab1_content(self, parent_tab):
        """SEKME 1: Manuel Kontrol - Düzeltilmiş Hali"""

        # --- DÜZELTME: 'tagged_frame' artık 'self' ile başlıyor ---
        self.tagged_frame = ttk.LabelFrame(
            parent_tab,
            text=self.strings.get("tab1_tagged_frame", "Etiketli Kontrol (Hızlı Seçim)"),
            padding="10"
        )
        self.tagged_frame.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        # --- DÜZELTME: Label'lar artık 'self' ile başlıyor ---
        ...
        # --- Bağlantı Kontrolü (Değişiklik yok) ---
        self.conn_frame = ttk.LabelFrame(
            parent_tab, text=self.strings.get('conn_frame_title', "Bağlantı"), padding="10"
        )
        self.conn_frame.grid(row=0, column=1, padx=10, pady=10, sticky="ew")
        self.connect_button = ttk.Button(
            self.conn_frame, text=self.strings.get('connect_button', "Bağlan"),
            command=self.connect_to_server
        )
        self.connect_button.pack(side="left", padx=5)
        self.disconnect_button = ttk.Button(
            self.conn_frame, text=self.strings.get('disconnect_button', "Bağlantıyı Kes"),
            command=self.disconnect_from_server
        )
        self.disconnect_button.pack(side="left", padx=5)

        # --- Holding Register Okuma/Yazma (Değişiklik yok) ---
        self.hr_frame = ttk.LabelFrame(
            parent_tab, text=self.strings.get('hr_frame_title', "Holding Register"), padding="10"
        )
        self.hr_frame.grid(row=1, column=0, padx=10, pady=10, sticky="ew")

        self.hr_addr_label = ttk.Label(self.hr_frame, text=self.strings.get('address_label', "Adres"))
        self.hr_addr_label.pack(side="left", padx=5)
        self.hr_addr_entry = ttk.Entry(self.hr_frame, width=8)
        self.hr_addr_entry.pack(side="left", padx=5)

        self.hr_val_label = ttk.Label(self.hr_frame, text=self.strings.get('value_label', "Değer"))
        self.hr_val_label.pack(side="left", padx=5)
        self.hr_val_entry = ttk.Entry(self.hr_frame, width=10)
        self.hr_val_entry.pack(side="left", padx=5)

        self.hr_read_btn = ttk.Button(
            self.hr_frame, text=self.strings.get('read_button', "Oku"),
            command=self.read_holding_register
        )
        self.hr_read_btn.pack(side="left", padx=5)
        self.hr_write_btn = ttk.Button(
            self.hr_frame, text=self.strings.get('write_button', "Yaz"),
            command=self.write_holding_register
        )
        self.hr_write_btn.pack(side="left", padx=5)

        self.reg_read_val_label = ttk.Label(self.hr_frame, text=self.strings.get('read_value_label', "Okunan Değer"))
        self.reg_read_val_label.pack(side="left", padx=5)

        # --- Coil Okuma/Yazma (Değişiklik yok) ---
        self.coil_frame = ttk.LabelFrame(
            parent_tab, text=self.strings.get('coil_frame_title', "Coil"), padding="10"
        )
        self.coil_frame.grid(row=1, column=1, padx=10, pady=10, sticky="ew")

        self.coil_addr_label = ttk.Label(self.coil_frame, text=self.strings.get('address_label', "Adres"))
        self.coil_addr_label.pack(side="left", padx=5)
        self.coil_addr_entry = ttk.Entry(self.coil_frame, width=8)
        self.coil_addr_entry.pack(side="left", padx=5)

        self.coil_stat_label = ttk.Label(self.coil_frame, text=self.strings.get('status_label', "Durum"))
        self.coil_stat_label.pack(side="left", padx=5)
        
        # --- ÖNEMLİ: Coil Değer Combobox'ı eksikti, eklendi ---
        self.coil_val_combo = ttk.Combobox(self.coil_frame, values=["True", "False"], width=7, state="readonly")
        self.coil_val_combo.pack(side="left", padx=5)
        self.coil_val_combo.set("True")
        # --- BİTTİ ---

        self.coil_read_btn = ttk.Button(
            self.coil_frame, text=self.strings.get('read_button', "Oku"),
            command=self.read_coil
        )
        self.coil_read_btn.pack(side="left", padx=5)
        self.coil_write_btn = ttk.Button(
            self.coil_frame, text=self.strings.get('write_button', "Yaz"),
            command=self.write_coil
        )
        self.coil_write_btn.pack(side="left", padx=5)

        self.coil_read_val_label = ttk.Label(self.coil_frame, text=self.strings.get('read_status_label', "Okunan Durum"))
        self.coil_read_val_label.pack(side="left", padx=5)
        
        # Ekipmanları combobox'a doldur
    ...
    def _load_alarm_rules_to_view(self):
        #"""Veritabanındaki tüm alarm kurallarını okur ve alttaki tabloyu doldurur."""
        for item in self.alarm_rules_tree.get_children():
            self.alarm_rules_tree.delete(item)
        try:
            # --- YENİ EKLENDİ: Çeviri için bir harita oluştur ---
            priority_map = {
            ...
            for rule in rules:
                # --- GÜNCELLENDİ: Sabit metinler yerine dil dosyasından oku ---
                is_active_str = self.strings.get("generic_yes") if rule[5] == 1 else self.strings.get("generic_no")
                # Öncelik değerini haritadan çevir
                priority_str = priority_map.get(rule[4], rule[4])
                # --- BİTTİ ---
                self.alarm_rules_tree.insert("", "end", values=(rule[0], rule[1], rule[2], rule[3], priority_str, is_active_str))
            conn.close()
        except Exception as e:
    ...
    def _update_active_alarms_view(self, new_active_alarms):
        ...
        # Eğer aktif bir alarm varsa durum çubuğunu uyaracak şekilde güncelle
    ...
    def open_settings_window(self):
        ...
        def _save_settings():
            ...
            if old_lang != self.language:
                self._load_language()
                self._create_menubar() # Menüyü yeniden çiz
                self._update_all_ui_text() # Diğer tüm metinleri güncelle
            self.update_status(self.persistent_status[0], self.persistent_status[1], is_persistent=True)
    ...
    def update_status(self, message, status_type='normal', is_persistent=False):
        """Durum çubuğunu istenen mesaj ve renkle günceller."""
        # Eğer bir zamanlayıcı varsa önce iptal et
        ...
        if hasattr(self, 'status_label'):
            ...
            # Aktif temaya göre doğru rengi renk paletinden seç
            color = self.colors[self.theme][status_type]
            self.status_label.config(text=f"{status_prefix}: {message}", foreground=color)
            self.last_status = (message, status_type)
            
            # Eğer mesaj kalıcı ise, bunu varsayılan durum olarak sakla
            if is_persistent:
                self.persistent_status = (message, status_type)
            # Kalıcı değilse, 5 saniye sonra varsayılan duruma geri dön
            else:
    ...
    def _update_live_view(self, view_type):
        """
        'Veri Görüntüleme' sekmesindeki canlı veri tablosunu, seçilen veri tipine göre günceller.
        Artık sadece son sütunun başlığını günceller ve verileri doldurur.
        """
        if not self.check_connection():
            return
        if not hasattr(self, 'live_tree') or not self.live_tree.winfo_exists():
            return
            ...
            if view_type == 'coil':
                ...
                if not result.isError():
                    for i, val in enumerate(result.bits[:count]):
                        tag_info = self.tag_cache.get(("Coil", i), {})
                        cihaz = tag_info.get('cihaz_adi', '')
                        etiket = tag_info.get('etiket_adi', '')
                        self.live_tree.insert("", "end", values=(cihaz, etiket, i, str(val)))
                else:
                ...
                if not result.isError():
                    for i, val in enumerate(result.registers):
                        tag_info = self.tag_cache.get((reg_type_str, i), {})
                        cihaz = tag_info.get('cihaz_adi', '')
                        etiket = tag_info.get('etiket_adi', '')
                        self.live_tree.insert("", "end", values=(cihaz, etiket, i, val))
                else:
